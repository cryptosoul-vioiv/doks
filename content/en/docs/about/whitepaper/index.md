...
Therefore, in the Famlan Metaverse, all NFTs that condense elements of human spirit, consciousness, culture, art, etc. can fertilize Famland. Including but not limited to encrypted trees, encrypted crops, encrypted real estate, encrypted animals and plants, etc.
...
- The Famland that participated in the merger does not support the Famland that has already made markets in Original Field and Wild Field for the time being.

- For Famland participating in the merger, the market-making principal value of Famland's basic field must reach 500USDT.

- You must use 1 Nephrite NFT each time you merge Famland, and pay merge handling fee.

#### Famland Merge Rules

| Merge method |   Use Nephrite    | Success rate | Shared Values after merged |
| :------: | :-------------: | :----: | :------------: |
...
The merged shared value means that the final merged Famland shared value is the shared value of multiple Famland blocks. After the merge is completed, the finally obtained Famland is added and then multiplied by the corresponding coefficient.
#### Consolidation fee

30YL will be charged each time for the merged, 60% will enter the YLE farm shared fund pool, 20% will enter the reserve fund, and 20% will enter the stable pool.

#### Famland Merge Results

- Regardless of the success or failure of the merger, the Nephrite NFT used by merged will be recovered into the YLE farm shared fund pool, and will continue to be collected and distributed again.
- After the merger is successful, the corresponding number of Famland will be generated according to the Hetian rules. The basic field of each Famland will automatically deposit 500U of market-making funds, and the shared value will also be updated to the latest value according to the rules.
- The merged Famland base field market-making funds will all be transferred to the YLE Farm Sharing Fund, and the merged Famland will be destroyed.

---


###  Collection of Nephrite

#### Collection requirements

For Famland whose basic field is over 500USDT, each rebase cycle will add a collection opportunity of nephrite rough. If the collection opportunity is missed, the collection opportunity will be invalid.
#### Collection fee

In the first farming cycle, the collection fee is 30YL. After that, the collection fee for each rebase cycle is increased by 0.1YL on the basis of the previous rebase cycle.
60% of the collection fee goes into the YLE farm shared fund pool, 20% goes into the YLE reserve fund pool, and 20% goes into the YLE stable pool.
The renter collects Nephrite on behalf of the farmer, and is paid from the farmer's fund where Famland is located.

#### Collection rules

The following is the proportion of nephrite that can be collected by all eligible players in each farming cycle.

| Collection times |                        Proportion of collecting Nephrite                        |
| :------: | :----------------------------------------------------------: |
|   0-4    |                        gray jade 100%                        |
...
Only the farmer of the shared farm can initiate a basic land lease contract. During the valid lease period, the lessee and the Famland owner will share the income from the cultivation of the basic land part of Famland according to the share of the lease contract. The income from the cultivation of the original fields and wild fields of the Famland still belongs to the farmers.
**Contents of the rental contract**

The lease contract will stipulate Famland's lease term, revenue sharing ratio, revenue freezing ratio, Hetian jade collection reward rules, lease contract termination conditions and other contents.
Once the farmer and the tenant have determined the lease agreement, the content of the lease cannot be amended until the lease agreement expires. After the lease contract is initiated for the same field, the lease contract cannot be initiated again, and the lease must not be initiated again until the end of the lease period.

**Rental contract initiation fee**

When the farmer generates the lease contract, he will pay the handling fee according to the number of rented Famland. The fee standard is: 30YL/Famland, 60% into the YLE farm shared fund pool, 20% into the reserve fund pool, and 20% into the stable pool.
---
...
Invariant slippage is an uncertain transaction slippage value generated when the real-time transaction price of YL deviates from the benchmark price (including ±0.02USDT) during the SWAP transaction process of the USDT/YL LP farm pool in YLE. The farther the price deviates from the benchmark price, the higher the uncertainty value that may be generated by impermanent slippage, and the higher the fee for impermanent slippage.

> For example, when the current benchmark price of YL is 1.00USDT and the real-time trading price of YL is 1.02USDT, the normal transaction slippage of the buyer is 0.03%. The invariant slippage is displayed as 0.1%-1%. However, when the transaction price is 1.05USDT, the normal transaction slippage is still 0.03%, while the irregular slippage is displayed as 0.4%-4%. That is to say, at this time, the minimum total slippage generated may be 0.43%, and the maximum slippage will be 4.03%. If the price rises to 1.06USDT, the maximum total slippage may reach 8.03%. And so on.
...
**Invariant slippage fee bearer**

When the transaction price of YL is higher than the benchmark price by 0.02USDT or more, the buyer accepts no constant slippage fee during the transaction, and the seller's transaction is not affected; on the contrary, when the transaction price of YL is lower than the benchmark price by 0.02USDT or less, The seller bears the invariable slippage fee during the transaction, and the buyer's transaction is not affected.
**Inconstant slippage value algorithm rules**
...
1. 80% of the income when the YLS stable share points are auctioned
2. 80% of transaction fees in the YLE ecosystem
3. 80% of transaction fee without constant slippage
4. 4% of the user's farming income is allocated from the reservoir
5. 80% of the handling fee when adjusting the pledged funds in basic field

### SWAP Stable Pool Bonus Pool

**Stable bonus pool incentive mechanism**

The function of the stable bonus pool is to motivate YL buyers during this period when the price of YL is lower than the benchmark price, so as to ensure that the price of YL rises back to the normal benchmark price range of YL.

**Source of funding for stable bonus pools** 

1. 18% of the income when the YLS stable share points are auctioned
2. 20% of transaction fees in the YLE ecosystem
3. 20% of transaction fee without constant slippage
4. 1% of the user's farming income is allocated from the reservoir
5. 20% of the handling fee when adjusting the pledged funds in basic field
...
When the price of YL falls below the minimum value of the benchmark price range, using USDT to exchange YL in the farm SWAP fund pool will inevitably get regular bonus rewards, and may also get random bonus rewards.

**Regular bonus award rules**

Regular bonus = USDT paid by homebuyers to purchase YL × (fund balance of regular bonus pool in the current cycle / (YL transaction amount in the previous cycle × 3))
...
If the purchase funds of YL in this cycle are too large and exceed 3 times the transaction amount of the previous cycle, the random bonus will be proportionally reduced to ensure the distribution of regular bonuses.
...
The undistributed part of the regular bonus in each cycle and the unsettled part will be accumulated into the regular bonus pool of the next cycle.

**Random bonus reward rules:**

When the home buyer settles the regular bonus in the previous cycle, there will be a chance to draw a random bonus.

Each cycle will divide the total random bonus of the current cycle into several equal parts. Among them, there is 1 special prize with 15% bonus; 3 first prizes with 15% bonus; 9 second prizes with 15% bonus; 30 third prizes with 15% bonus; 300 incentive awards with bonus 40%.
...
Farm handling fees: including Famland rental handling fees, merge handling fees, etc.;

Farm combined field fee: including the collection fee of nephreite merged.
...
- Nephrite merged Rewards (30% bonus)


**Farm Shared Value Ranking Rewards**

**Farm Shared Value Ranking (Bonus Percentage: 22%)**
...
**Famland Shared Value Average Ranking (Bonus Proportion: 26%)**

The ranking is rewarded to the top 10 farmers in the average farm share value of each Rebase cycle.
...
In each Rebase cycle, home buyers who perform basic field merge operations can have the opportunity to get rewards. Regardless of whether the market-making capital is full or not, the probability of winning is calculated based on the famland merge handling fee. 
...
**Yield Land Share (YLS)**
YLS (Yield land share), also known as the original grain shared gold. The limited issuance of 1 million pieces is a kind of shared points with certain risks and benefits to ensure the stability of YL's value. Home buyers can obtain YLS by participating in the YLS auction.
...
#### Issuance of YL
The YL issuance system is to ensure the stability of YL's value creation. The value of YL is not created out of nothing, but based on people-oriented, relying on the value elements created by people and people to support.

Except for the pre-release of 1.5 million YL to the community-designated airdrop wallet account (multi-signature wallet, subject to community supervision) due to the needs of airdrop activities in the initial stage of the launch of the YLE project, there is no reservation, and other parts are all generated through the issuance of smart contracts. Divided into four eras.
...
The calculation standard for famland reclamation is that the home buyer successfully pledges liquidity funds in the basic plots of the famland.
...
All YL issued successfully will be injected into the YLE ecological farm reservoir. Then, through the flow rate control module, YL is transferred to the YLE total distribution pool, stable fund pool and stable bonus pool. Under the premise of meeting the peak limit, the peak annualized APY of all YLE farms is 3000%.

The closer the price of YL is to the benchmark price, the greater the flow rate flowing into the distribution pool, which will increase the circulation of YL in the market, and the higher the farm APY. Conversely, the more YL price deviates from the benchmark price, the smaller the flow rate into the distribution pool, which will reduce the YL circulation in the market, and the lower the farm's APY will be. Especially when the price of YL is lower than the benchmark price, the proportion of YL flowing into the distribution pool will decrease more obviously, and a larger proportion of YL will flow to the stable pool fund and the stable bonus pool at this time.
...
**When YL is trading above the benchmark price (inclusive)**

The proportion of transfer into the total distribution pool = 95% × YL price correction parameter 2
...
**Maximum flow rate = last farming cycle YLE reservoir YL balance × 30%/86400**
...
The default value of the flow rate coefficient of the shared farm is 1. For every additional 30 pieces of Famland that have already made a market in the basic farm, the flow rate coefficient will increase by 0.0001.

**sub-farm flow rate**

**Sub-farm flow rate = sub-farm flow rate index / YLE ecological farm total flow rate**

According to the above algorithm rules, it can be known that Famland with high sharing value will move out of the original farm in time to build a shared farm. In the end, the only ones who stay on the native farm are those Famland whose shared value is very low, and any shared farm is unwilling to accept it. There is no doubt that these Famland who stay on the original farm will only get lower and lower income in the future.



**distribution pool**

The flow rate control module flows into the distribution pool of each sub-farm according to the flow rate of each sub-farm. At the same time, every time the home buyer receives the income, it will reduce the balance of the distribution pool of the farm where it is located, thereby changing the APY of the current farm.
...
YLS (Yield land share), also known as the original grain share bonus. The limited issuance of 1 million pieces is a kind of shared points with certain risks and benefits to ensure the stability of YL's value.

#### Share bonus 

In each Rebase farming cycle, the shared bonus generated by YLE will be evenly distributed to each YLS according to 1 million YLS. When users obtain YLS through auction, they can also immediately get each YLS accumulated shared bonus, and at the end of each period in the future, they can also receive the shared bonus dividend for that period according to the proportion of the YLS held.
...
80% of YLS auction revenue enters the YLE stable fund pool, 18% enters the YLE stable bonus pool, and 2% enters the YLE technology development fund special account.
